...
A coverage of $\pi$ calculus is on \ref{parrow}, \ref{sangiorgiwalker} and \ref{milner}
...
\begin{definition} \index{$bn$}
  $bn(P)$ is the set of names that have a bound occurrence in $P$ and is defined as $B(P, \emptyset)$, where $B(P, I)$, with $I$ a set of process constants, is defined in table \ref{B}
\end{definition}

  \begin{table}[t]\label{B}
    \begin{tabular}{ll}
      \hline\\
	...
	$B(A(x_{1},\cdots, x_{n}), I)=\left\{
	  ...
	  \end{array}\right.$}
      \\\hline
    \end{tabular}\caption{Bound occurrences}
  \end{table}
...
\end{definition}
  ...
  \begin{table}\label{F}
	...
	$F(A(x_{1},\cdots, x_{n}), I)=\left\{
      ...
      \\\hline
    \end{tabular}\caption{Free occurrences}
  \end{table}
...
\end{definition}
  ...
  \begin{table}\label{syntacticsubstitution}
    \begin{tabular}{l}
      \hline\\
	$0\{b/a\}\; =\; 0$
      \\\\
	...
	$((\nu b)Q)\{b/a\}\; =\;(\nu b)Q$ if $a\notin fn(Q)$
      \\\hline
    \end{tabular}\caption{Syntatic substitution}
  \end{table}
...
\begin{definition}\index{structural congruence}
  ...
  \begin{table}\label{structuralcongrunce}
    \begin{tabular}{lll}
      \hline\\
	...
	SC-IDE&$A(\tilde{y})\equiv P\{\tilde{y}/\tilde{x}\}$&if $A(\tilde{x})\stackrel{def}{=}P$
      \\\hline
    \end{tabular}\caption{Structural congruence axioms}
  \end{table}
\end{definition}
...
\begin{definition}\index{transition relation! early! without structural congruence}
  ...
  \begin{table}\label{transitionrelationearlywithoutstructuralcongruence}
    \begin{tabular}{ll}  
      \hline\\
	  \bf{Out}
	  \begin{tabular}{c}
	...
	&
	  ...
	  \end{tabular}
      \\\hline
    \end{tabular}\caption{Early transition relation without structural congruence}
  \end{table}
\end{definition}
