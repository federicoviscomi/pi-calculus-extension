...
\begin{description}
  ...
  \item[$A(x_{1}, \cdots, x_{n})$] 
    ...
    \begin{center}
      $A(x_{1}, \cdots, x_{n})=P$
    \end{center}
    the $x_{i}$s must be pairwise disjoint. The intuition is that we can substitute for some of the $x_{i}$s in $P$ to get a $\pi$ calculus process. 
\end{description}
  ...
  \begin{table}
    ...
    \\\\
	  $\inferrule*[left=AlpRes2]{
	    ...
	    (\nu x)(P\{x/z\})\equiv_{\alpha}(\nu y)(Q\{y/z\})
	  }$
	&
      \\\\
	  $\inferrule*[left=AlpRes3]{
	      P\{z/x\}\equiv_{\alpha}Q\{z/y\}
	    \\
	      z\notin fn(P)
	    \\
	      z\notin fn(Q)
	...
	&
%     \\\\
% 	  $\inferrule*[left=AlpSubst]{
% 	      P\equiv_{\alpha}Q
% 	    \\
% 	      a\leq b
% 	  }{
% 	    P\{a/b\}\equiv_{\alpha}Q\{a/b\}
% 	  }$
% 	&
      \\\hline
    \end{tabular}
...
\begin{lemma}
  $\alpha$ equivalence is invariant with respect to substitution. In other words if we have a proof tree of 
  \[
    P\equiv_{\alpha}Q
  \]
  ...
  \begin{proof}:
	...
	\begin{description}
	  ...
	  \item[$AlpInp$]
	    ...
	    \end{center}	    	    
	  \item[$AlpInp1$]:
	    Let $x,y,z,a$ and $b$ be pairwise disjoint, then we have various cases:
	    \begin{itemize}
	      \item 
		...
		rule $AlpInp$
	      \\
		$\Rightarrow z(x).(P_{1}\{b/a\})\equiv_{\alpha}z(x).(Q_{1}\{x/y\}\{b/a\})$
		  &
		definition of substitution
	      \\
		$\Rightarrow (z(x).P_{1})\{b/a\}\equiv_{\alpha}(z(x).(Q_{1}\{x/y\}))\{b/a\}$
		  &
		transitivity and equation (*)
	      \\
		$\Rightarrow (z(x).P_{1})\{b/a\}\equiv_{\alpha}(z(y).Q_{1})\{b/a\}$
	      ...
	      \end{tabular}
	    \end{center}	    	
%  	    we create the following proof 
%  	    \[
%  	      \inferrule* [left=AlpInp1]{
%  		\inferrule* [left=AlpRef]{
%  		}{
% 		  Q_{1}\{x/y\}\equiv_{\alpha}Q_{1}\{x/y\}
%  		}
%  	      }{
%  		z(x).(Q_{1}\{x/y\})\equiv_{\alpha}z(y).Q_{1}
%  	      }
%  	    \]
% 	    which is shorter than the proof of $z(x).P_{1}\equiv_{\alpha}z(y).Q_{1}$ if the proof of $P_{1}\equiv_{\alpha}Q_{1}\{x/y\}$ is longer than one, so we can apply the inductive hypothesis and get: 
% 	    \begin{center}
% 	      \begin{tabular}{ll}
%  		  &
%  		reflexivity
...
%  		rule $AlpInp1$
%  	      \\
%  		$\Rightarrow z(x).(Q_{1}\{x/y\})\equiv_{\alpha}z(y).Q_{1}$
%  		  &
%  		inductive hypothesis POSSIAMO?
%  	      \\
% 		$(z(x).(Q_{1}\{x/y\}))\{b/a\}\equiv_{\alpha}(z(y).Q_{1})\{b/a\}$ 
% 		  &
% 		(*)
% 	      \\				
% 	      \end{tabular}
% 	    \end{center}
% 	    if the length of the proof of $P_{1}\equiv_{\alpha}Q_{1}\{x/y\}$ is one then the rule used can be
% 	    
% 	    $AlpZero$ $AlpIde$ or $AlpRef$
	    ...
	    \begin{center}
	      \begin{tabular}{ll}
		&rule premise\\
		$P_{1}\equiv_{\alpha}Q_{1}\{a/y\}$ and $y\in fn(Q_{1})$ and $a\notin fn(Q_{1})$&\\
		$?$&\\
	    ...
	    \begin{center}
	      \begin{tabular}{ll}
		&rule premise\\
		$P_{1}\equiv_{\alpha}Q_{1}\{b/y\}$ and $y\in fn(Q_{1})$ and $b\notin fn(Q_{1})$ &\\
		$?$&\\
	    ...
	    \begin{center}
	      \begin{tabular}{ll}
		&rule premise\\
		$P_{1}\equiv_{\alpha}Q_{1}\{b/a\}$ and $a\in fn(Q_{1})$ and $b\notin fn(Q_{1})$ &\\
		$?$&\\
		$\Rightarrow (a(b).P_{1})\{b/a\}\equiv_{\alpha}(a(a).Q_{1})\{b/a\} $&\\
	      \end{tabular}
	    \end{center}	    	    
	    \begin{center}
	      \begin{tabular}{ll}
		&rule premise\\
		$P_{1}\equiv_{\alpha}Q_{1}\{x/a\}$ and $a\in fn(Q_{1})$ and $x\notin fn(Q_{1})$ &\\
		$?$&\\
	    ...
	    \begin{center}
	      \begin{tabular}{ll}
		&rule premise\\
		$P_{1}\equiv_{\alpha}Q_{1}\{x/b\}$ and $b\in fn(Q_{1})$ and $x\notin fn(Q_{1})$ &\\
		$?$&\\
	    ...
	    \begin{center}
	      \begin{tabular}{ll}
		&rule premise\\
		$P_{1}\equiv_{\alpha}Q_{1}\{a/b\}$ and $b\in fn(Q_{1})$ and $a\notin fn(Q_{1})$ &\\
		$?$&\\
		$\Rightarrow (a(a).P_{1})\{b/a\}\equiv_{\alpha}(a(b).Q_{1})\{b/a\} $&\\
	      \end{tabular}
	    \end{center}	    	    
	  ...
	  \item[$AlpRes$]:
	    ...
	    \end{center}	    	    
	  \item[$AlpRes1$]:
	    Since $a$ is in the free names of both $P$ and $Q$ then $a$ cannot be in the restriction at the top level of $P$ or $Q$. So let $x,y,b$ and $a$ be pairwise disjoint. We have three different cases:
	    ...
	    \end{description}
	    \[\inferrule*[left=AlpRes1]{
		P_{1}\equiv_{\alpha}Q_{1}\{b/y\}
	      \\
		b\neq y
	      \\
		b\notin fn(Q_{1})
	    }{
	      \underbrace{(\nu b)P_{1}}_{P}\equiv_{\alpha}\underbrace{(\nu y)Q_{1}}_{Q}
	    }\]

	    we have to prove that
	    \[
	     	P_{1}\{b/a\}\equiv_{\alpha}Q_{1}\{/y\}\{b/a\}
	      \mbox{ and }
		b\neq y
	      \mbox{ and }
		b\notin fn(Q_{1}) \Rightarrow 
	    ...
	    we have to prove that
	    \[
	     	P_{1}\{b/a\}\equiv_{\alpha}Q_{1}\{x/b\}\{b/a\}
	      \mbox{ and }
		x\neq b
	      \mbox{ and }
		x\notin fn(Q_{1}) \Rightarrow 
...
\begin{lemma}
  $P\{x/y\}\equiv_{\alpha}Q$ if and only if $P\equiv_{\alpha}Q\{y/x\}$. NON FUNZIONA LA DIMOSTRAZIONE! staro' forse esagerando?
  \begin{proof}
...
\begin{lemma}
  ...
  \begin{proof}:
	...
	suppose 
	\[
	  P\equiv_{\alpha}Q \mbox{ and } Q\equiv_{\alpha}R
	\]
	...
	\begin{description}
	  \item[$AlpInp$]
	    $P_{1}\equiv_{\alpha}Q_{1}$ rule premise
	    $x(y).Q_{1}\equiv_{\alpha}R$ implies for symmetry and the inversion lemma that 
	    \begin{itemize}
	      \item 
		$R=x(y).R_{1}$ and $Q_{1}\equiv_{\alpha}R_{1}$
	      \item 
		$R=x(z).R_{1}$ and $Q_{1}\equiv_{\alpha}R_{1}\{z/y\}$
	    \end{itemize}
	  \item[$AlpRes$]
  ...
  \begin{proof}
	...
	\begin{description}
	  ...
	  \item[EInp]
		...
		\begin{center}
		  \begin{tabular}{ll}
		    $P=x(w).P_{1}$ and $P_{1}\equiv_{\alpha}Q_{1}\{w/y\}$& rule $EInp$\\
		    $\Rightarrow x(w).P_{1}\xrightarrow{x z}_{1}P_{1}\{z/w\}$& \\
		  \end{tabular}
		\end{center}
		This is what we want because 
		\[
		  P_{1}\equiv_{\alpha}Q_{1}\{w/y\} \Rightarrow P_{1}\{z/w\}\equiv_{\alpha}Q_{1}\{w/y\}\{z/w\}\equiv_{\alpha}Q_{1}\{z/y\}
		\]		
	    \end{itemize}
	  \item[Tau]
