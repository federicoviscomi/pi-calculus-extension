...
\begin{definition}\index{transition relation! early! without structural congruence}
  ...
  \begin{table}
	...
	&
	  ...
	  \bf{CnsAlp}
	  \begin{tabular}{c}
	      $A(\tilde{x}|\tilde{y}) \stackrel{def}{=} P\; P\{\tilde{w}/\tilde{x}\}\{\tilde{z}/\tilde{y}\}_{bound} \xrightarrow{\alpha} P^{'}$
	    \\\hline
	      $A(\tilde{w}|\tilde{z}) \xrightarrow{\alpha} P^{'}$
	  \end{tabular}
      \\\\
	  \bf{Opn}
	  \begin{tabular}{c}
...
\begin{lemma}
  :\begin{itemize}
	...
	\item
      ...
      \end{itemize}
    \item 
      If $P\equiv_{\alpha} A(\tilde{x}|\tilde{y})$ then ??? ?? ?
  \end{itemize}
  \begin{proof}
