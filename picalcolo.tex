...
\begin{description}
  ...
  \item[$\alpha\; conversion$]
    is the $\alpha$ conversion, i.e., the choice of bound names, it identifies agents like $x(y).\overline{z}y$ and $x(w).\overline{z}w$. In the semantic of $\pi$ calculus we can use the structural congruence with the rule $Alp$ or we can embed the $\alpha$ conversion in the SOS rules. 
...
%  	  \end{tabular}
%  	\end{center}
%  	for lemma \ref{alphaequivalencesubstitution} and inductive hypothesis: $P\{c/b\}\equiv_{\alpha} R$ and for rule $AlpInp2$: 
% 	\begin{center}
% 	  $a(b).(P\{c/b\}\{b/c\})\equiv_{\alpha} a(c).R$ 
% 	\end{center}
% 	which imply for lemma \ref{alphaequivalenceSubstitutionInversion} and rule $AlpInp1$: $a(b).P \equiv_{\alpha} a(c).R$
%       \item[$(AlpInp, AlpInp2)$]:
% 	\begin{center}
% 	  \begin{tabular}{ll}
% 	      $\inferrule* [left=\bf{AlpInp}]{
...
% 		R \equiv_{\alpha} S
% 	      }{
% 		a(b).(R\{b/c\}) \equiv_{\alpha} a(c).S
% 	      }$
% 	  \end{tabular}
% 	\end{center}
% 	where $Q=R\{b/c\}$. For lemma \ref{alphaequivalencesubstitution} and inductive hypothesis: $P\equiv_{\alpha} S\{b/c\}$ which imply for rule $AlpInp1$: $a(b).P \equiv_{\alpha} a(c).(S\{b/c\}\{c/b\})$ which imply $a(b).P \equiv_{\alpha} a(c).S$
...
% \subsection{Alternative definition of $\alpha$ equivalence}
% 
% \begin{definition}
%   $P\{b/a\}_{nb}$ is the syntactic substitution of name $b$ for a different name $a$ inside a $\pi$ calculus process but it differs from the previous definition in that this substitution does not change any bound name. When it reaches a bound name that would capture $b$, it stops. $P\{b/a\}_{nb}$ is defined in table \ref{syntacticsubstitutionNoChangeOfBoundNames}.
%   \begin{table}
%     \begin{tabular}{lll}
%       \multicolumn{3}{l}{\line(1,0){415}}\\\\
% 	$0\{b/a\}_{nb} = 0$
%       &
% 	$(\overline{x}y.Q)\{b/a\}_{nb} = \overline{x}\{b/a\}y\{b/a\}.(Q\{b/a\}_{nb})$
%       &
% 	$(\tau.Q)\{b/a\}_{nb} = \tau.(Q\{b/a\}_{nb})$
%       \\
%     \end{tabular}
%       \\
%     \begin{tabular}{l}
...
% 	    y\neq b
% 	}{
% 	  (x(y).Q)\{b/a\}_{nb} = x\{b/a\}(y).(Q\{b/a\}_{nb})
% 	}$
%       \\
%     \end{tabular}
%       \\
%     \begin{tabular}{ll}
%       \\
% 	$(x(a).Q)\{b/a\}_{nb} = x\{b/a\}(a).Q$
%       &
% 	$(x(b).Q)\{b/a\}_{nb} = x\{b/a\}(c).Q$
%       \\
%     \end{tabular}
%       \\
%     \begin{tabular}{l}
...
% 	    A(\tilde{x}) \stackrel{def}{=} P
% 	}{
% 	    A(\tilde{x})\{b/a\}_{nb} = A(\tilde{x}\{b/a\})
% 	}$
%       \\
%     \end{tabular}
%       \\
%     \begin{tabular}{ll}
%       \\
% 	$(Q+R)\{b/a\}_{nb} = Q\{b/a\}_{nb} + R\{b/a\}_{nb}$
%       &
% 	$(Q|R)\{b/a\}_{nb} = Q\{b/a\}_{nb} | R\{b/a\}_{nb}$
%       \\
%     \end{tabular}
...
%       \\\multicolumn{3}{l}{\line(1,0){415}}
%     \end{tabular}
%     \caption{Syntatic substitution without change of bound names}
%     \label{syntacticsubstitutionNoChangeOfBoundNames}
%   \end{table}
% 
% \end{definition}
% 
% 
% \begin{definition}
%   Processes $P$ and $Q$ are \emph{$\alpha$ convertible} or \emph{$\alpha$ equivalent} if $Q$ can be obtained from $P$ by a finite number of changes of bound names. If $P$ and $Q$ are $\alpha$ equivalent then we write $P \equiv_{\alpha}^{nb}Q$. Specifically the $\alpha$ equivalence is the smallest binary relation on processes that satisfies the laws in table \ref{alphaequivalenceNoChangeOfBoundNames}.
%   \begin{table}
%     \begin{tabular}{lll}
%       \multicolumn{3}{l}{\line(1,0){415}}\\
% 	  $\inferrule*[left=\bf{AlpOut}]{
% 	      P \equiv_{\alpha}^{nb} Q
% 	  }{
% 	    \overline{x}y.P \equiv_{\alpha}^{nb} \overline{x}y.Q
% 	  }$
% 	&
% 	  $\inferrule*[left=\bf{AlpTau}]{
% 	      P \equiv_{\alpha}^{nb} Q
% 	  }{
% 	    \tau.P \equiv_{\alpha}^{nb} \tau.Q
% 	  }$
% 	&
% 	  $\inferrule*[left=\bf{AlpInp}]{
% 	      P \equiv_{\alpha}^{nb} Q
% 	  }{
% 	    x(y).P \equiv_{\alpha}^{nb} x(y).Q
% 	  }$
%       \\
%     \end{tabular}
%     \\
%     \begin{tabular}{lll}
%       \\
% 	  $\inferrule*[left=\bf{AlpIde}]{
% 	  }{
% 	    A(\tilde{x}) \equiv_{\alpha}^{nb} A(\tilde{x})
% 	  }$
% 	&
% 	  $\inferrule*[left=\bf{AlpZero}]{
% 	  }{
% 	    0 \equiv_{\alpha}^{nb} 0
% 	  }$
% 	&
...
% 	      P_{1} \equiv_{\alpha}^{nb} Q_{1}
% 	    \\
% 	      P_{2} \equiv_{\alpha}^{nb} Q_{2}
% 	  }{
% 	    P_{1}|P_{2} \equiv_{\alpha}^{nb} Q_{1}|Q_{2}
% 	  }$
%       &
% 	  $\inferrule*[left=\bf{AlpSum}]{
% 	      P_{1} \equiv_{\alpha}^{nb} Q_{1}
...
% 	      x\in fn(Q)
% 	  }{
% 	    z(x).P \equiv_{\alpha}^{nb} z(y).Q\{y/x\}_{nb}
% 	  }$
%       \\
%     \end{tabular}
%     \\
...
% 	      y\in fn(P)
% 	  }{
% 	    (\nu x)P\{x/y\}_{nb} \equiv_{\alpha}^{nb} (\nu y)Q
% 	  }$
%       \\\\
% 	  $\inferrule*[left=\bf{AlpInp2}]{
% 	      P \equiv_{\alpha}^{nb} Q
...
% 	      y\in fn(P)
% 	  }{
% 	    z(x).P\{x/y\}_{nb} \equiv_{\alpha}^{nb} z(y).Q
% 	  }$
%     \\\\\multicolumn{1}{l}{\line(1,0){415}}
%     \end{tabular}
%     \caption{$\alpha$ equivalence laws}
%     \label{alphaequivalenceNoChangeOfBoundNames}
%   \end{table}
% \end{definition}
% 
% \begin{proposition}
%   $\equiv_{\alpha}^{nb}$ is an equivalence relation.
%   \begin{proof}
%     
%   \begin{description}
%     \item[transitivity]
%   \end{description}
%   the proof is by induction on the depths of the derivation of $P \equiv_{\alpha}^{nb} Q$ and $Q \equiv_{\alpha}^{nb} P$ and by cases on the last pair of rule used:
%   \begin{description}
%     \item[$AlpRes, AlpRes1$]:
%       \begin{center}
% 	\begin{tabular}{ll}
...
% 	    Q \equiv_{alpha}^{nb} R
% 	  }{
% 	    (\nu y)Q\{y/x\}_{nb} \equiv_{alpha}^{nb} (\nu y)R
% 	  }$
% 	\end{tabular}
%       \end{center}
%   \end{description}
...
% 
% 
% \begin{lemma}\label{alphaequivalenceOfSubstitutions}
%   $P\{x/y\} \equiv_{\alpha}^{nb} P\{x/y\}_{nb}$.
%   \begin{proof}
%     
%   \end{proof}
% \end{lemma}
...
% 
% \begin{proposition}
%   $P\equiv_{\alpha} Q$ if and only if $P \equiv_{\alpha}^{nb} Q$.
%   \begin{proof}
%     \begin{description}
%       \item[$\Rightarrow$]
%     \end{description}
%     $P\equiv_{\alpha} Q$ imply $P \equiv_{\alpha}^{nb} Q$. The proof is an induction on the depth of the proof tree of $P \equiv_{\alpha} Q$ and by cases on the last rule used. If the last rule is in common between $\equiv_{\alpha}$ and $\equiv_{\alpha}^{nb}$ then the conclusion holds. Here is an example:
%     \begin{description}
%       \item[$AlpSum$]:
% 	\begin{center}
% 	  $\inferrule* [left=\bf{AlpSum}]{
...
% 	  }$
% 	\end{center}
% 	for inductive hypothesis $P_{1} \equiv_{\alpha}^{nb} Q_{1}$ and $P_{2} \equiv_{\alpha}^{nb} Q_{2}$. For rule $AlpSum$: $P_{1}+P_{2} \equiv_{\alpha}^{nb} Q_{1}+Q_{2}$
%     \end{description}
%     Otherwise the last rule is not in common and for example we can have:
%     \begin{description}
%       \item[$AlpRes1$]:
% 	\begin{center}
% 	  $\inferrule* [left=\bf{AlpRes1}]{
...
% 	  }$
% 	\end{center}
% 	for inductive hypothesis $P \equiv_{\alpha}^{nb} Q$. For rule $AlpRes1$: $(\nu x)P \equiv_{\alpha}^{nb} (\nu y)Q\{y/x\}_{nb}$. For lemma \ref{alphaequivalenceOfSubstitutions}: $Q\{y/x\}_{nb} \equiv_{\alpha}^{nb} Q\{y/x\}$, for rule $AlpRes$: $(\nu y)Q\{y/x\}_{nb} \equiv_{\alpha} (\nu y)Q\{y/x\}$. For transitivity $(\nu x)P \equiv_{\alpha}^{nb} (\nu y)Q\{y/x\}$.
%     \end{description}
...
%       \item[$\Leftarrow$]
%     \end{description}
%     $P \equiv_{\alpha}^{nb} Q$ imply $P\equiv_{\alpha} Q$.
%   \end{proof}
% \end{proposition}
% 
% \begin{proposition}
%   $P\{y/x\}_{nb}\{x/y\}_{nb}=P$.
%   \begin{proof}
%     It is an easy structural induction on $P$.
%   \end{proof}
% \end{proposition}
% 
% 
...
\subsection{Permutation semantic}
...
We would like to prove that $P\xrightarrow{\alpha}_{3}P^{'}$ imply that $P\xrightarrow{\alpha}_{2}Q^{'}$ and $Q^{'}\equiv P^{'}$ but this does not hold because:
\begin{center}
  $\inferrule*[left=\bf{Cong}]{
...
\begin{lemma}
  Let $\equiv_{2}$ be the smallest congruence defined by only the rules $ScpExtPar1$ and $ScpExtSum1$. If $P\equiv Q$ and $Q \xrightarrow{\alpha}_{2} Q^{'}$ then $P \equiv_{2} R \xrightarrow{\alpha}_{2} R^{'}$ and $R^{'} \equiv Q^{'}$.
  \begin{proof}
    The proof is an induction on the sum of the size of the derivations of $P\equiv Q$ and of $Q \xrightarrow{\alpha}_{2} Q^{'}$. First we notice that we can get rid ot the rules $ScpExtPar2$ and $ScpExtSum2$ because they can be mimicked by $ScpExtPar1$, $ScpExtSum1$ and the commutativity. In a similar way we can ignore the rules $SumAsc2$ and $ParAsc2$. The last pair of rules used can be:
    \begin{description}
      \item[$(SumAsc1, SumL)(1)$]:
	\begin{center}
	  \begin{tabular}{ll}
		...
		P_{1}+(P_{2}+P_{3})\equiv (Q_{1}+Q_{2})+Q_{3}
	      }$
	    &
	      $\inferrule* [left=\bf{SumL}]{
		\inferrule* [left=\bf{SumL}]{
		  Q_{1} \xrightarrow{\gamma}_{2}Q^{'}
		}{
		  Q_{1}+Q_{2} \xrightarrow{\gamma}_{2}Q^{'}
		}
	      }{
		(Q_{1}+Q_{2})+Q_{3} \xrightarrow{\gamma}_{2} Q^{'}
	      }$
	  \end{tabular}
	\end{center}
	for inductive hypothesis on $P_{1}\equiv Q_{1}\xrightarrow{\gamma}_{2}Q^{'}$: $P_{1} \equiv_{2} R_{1}$, $R_{1} \xrightarrow{\gamma}_{2} R_{1}^{'} $ and $Q^{'} \equiv R_{1}^{'}$. Then $P_{1}+(P_{2}+P_{3})\equiv_{2}R_{1}+(P_{2}+P_{3})$ and for rule $SumL$: $R_{1}+(P_{2}+P_{3})\xrightarrow{\gamma}_{2} R_{1}^{'}$.
      \item[$(SumAsc1, SumL)(2)$]:
	\begin{center}
	  \begin{tabular}{ll}
	      $\inferrule* [left=\bf{SumAsc1}]{
	    ...
	    &
	      $\inferrule* [left=\bf{SumL}]{
		\inferrule* [left=\bf{SumR}]{
		  Q_{2} \xrightarrow{\gamma}_{2}Q_{2}^{'}
		}{
		  Q_{1}+Q_{2} \xrightarrow{\gamma}_{2}Q_{2}^{'}
		}
	      }{
		(Q_{1}+Q_{2})+Q_{3} \xrightarrow{\gamma}_{2} Q_{2}^{'}
	      }$
	  \end{tabular}
	\end{center}
	similar.
	...
	\begin{center}
	  \begin{tabular}{ll}
		...
		P_{1}+P_{2}\equiv Q_{2}+Q_{1}
	      }$
	    &
	      $\inferrule* [left=\bf{SumL}]{
		Q_{2} \xrightarrow{\gamma}_{2}Q_{2}^{'}
	      }{
		Q_{2}+Q_{1} \xrightarrow{\gamma}_{2} Q_{2}^{'}
	      }$
	  \end{tabular}
	\end{center}
...
\begin{lemma}\label{sem2Extension}
...
% \begin{lemma}
%   If $P\equiv Q$ and $Q \xrightarrow{\alpha}_{2} Q^{'}$ then $P \xrightarrow{\alpha}_{2} P^{'}$ and $P^{'} \equiv Q^{'}$.
%   \begin{proof}
%     The proof is an induction on the size of the derivation of $P\equiv Q$, $Q \xrightarrow{\alpha}_{2} Q^{'}$ and by cases on the last pair of rules used:
%     \begin{description}
%       \item[$(SumAsc1, SumL)(1)$]:
% 	\begin{center}
% 	  \begin{tabular}{ll}
...
% 	  \end{tabular}
% 	\end{center}
% 	for inductive hypothesis on $P_{1}\equiv Q_{1}\xrightarrow{\gamma}_{2}Q^{'}$: $P_{1} \xrightarrow{\gamma}_{2} P^{'}\equiv Q^{'}$. Then for rule $SumL$: $P_{1}+(P_{2}+P_{3})\xrightarrow{\gamma}_{2} Q^{'}$.
%       \item[$(SumAsc1, SumL)(2)$]:
% 	\begin{center}
% 	  \begin{tabular}{ll}
% 	      $\inferrule* [left=\bf{SumAsc1}]{
...
% 	  \end{tabular}
% 	\end{center}
% 	for inductive hypothesis on $P_{2}\equiv Q_{2}\xrightarrow{\gamma}_{2}Q_{2}^{'}$: $P_{2} \xrightarrow{\gamma}_{2} P_{2}^{'}\equiv Q_{2}^{'}$. Then for rules $SumR$ and $SumL$: $P_{1}+(P_{2}+P_{3})\xrightarrow{\gamma}_{2} Q_{2}^{'}$.
%       \item[$(SumAsc1, SumR)$] similar to the cases $(SumAsc1, SumL)$.
%       \item[$(SumAsc2, \_)$] $SumAsc2$ is unnecessary because it can be emulated by $SumAsc1$ and $SumCom$.
%       \item[$(SumCom, SumL)$]:
% 	\begin{center}
% 	  \begin{tabular}{ll}
% 	      $\inferrule* [left=\bf{SumCom}]{
...
% 	    &
% 	      $\inferrule* [left=\bf{SumL}]{
% 		Q_{1} \xrightarrow{\gamma}_{2}Q_{1}^{'}
% 	      }{
% 		Q_{1}+Q_{2} \xrightarrow{\gamma}_{2} Q_{1}^{'}
% 	      }$
% 	  \end{tabular}
% 	\end{center}
% 	for inductive hypothesis on $P_{1}\equiv Q_{1} \xrightarrow{\gamma}_{2}Q_{1}^{'}$: $P_{1} \xrightarrow{\gamma}_{2} P_{1}^{'} \equiv Q_{1}^{'}$. The for rules $SumL$: $P_{1}+P_{2} \xrightarrow{\gamma}_{2} P_{1}^{'}$
%       \item[$(SumCom, SumR)$] similar to the previous case.
%       \item[$(ScpExtSum1, SumL)$]:
% 	\begin{center}
% 	  \begin{tabular}{ll}
% 	      $\inferrule* [left=\bf{ScpExtSum1}]{
% 		  P_{1}\equiv Q_{1}
...
% 		  x\notin fn(P_{1},Q_{1})
% 	      }{
% 		(\nu x)(P_{1}+P_{2})\equiv Q_{1}+(\nu x)(Q_{2})
% 	      }$
% 	    &
% 	      $\inferrule* [left=\bf{SumL}]{
% 		Q_{1} \xrightarrow{\gamma}_{2}Q_{1}^{'}
% 	      }{
% 		Q_{1}+(\nu x)(Q_{2}) \xrightarrow{\gamma}_{2} Q_{1}^{'}
% 	      }$
% 	  \end{tabular}
% 	\end{center}
% 	for inductive hypothesis on $P_{1}\equiv Q_{1} \xrightarrow{\gamma}_{2}Q_{1}^{'}$: $P_{1} \xrightarrow{\gamma}_{2} P_{1}^{'} \equiv Q_{1}^{'}$. Then for rules $SumL$: $P_{1}+P_{2} \xrightarrow{\gamma}_{2} P_{1}^{'}$
%       \item[$(ScpExtSum1, SumR)$]
%       \item[$(ScpExtSum2, SumL)$]
%       \item[$(ScpExtSum2, SumR)$]
%       \item[$(CongSum, SumL)$]:
% 	\begin{center}
% 	  \begin{tabular}{ll}
...
% 	  \end{tabular}
% 	\end{center}
% 	for inductive hypothesis on $P_{1} \equiv Q_{1} \xrightarrow{\gamma}_{2} Q_{1}^{'}$: $P_{1}\xrightarrow{\gamma}_{2} P_{1}^{'}$ and $P_{1}^{'} \equiv Q_{1}^{'}$. For rule $SumL$: $P_{1}+P_{2}\xrightarrow{\gamma}_{2} P_{1}^{'}$
%       \item[$(CongSum, SumR)$] similar to the previous case.
% 
%       \item[$(ParAsc1, ParL)$]
%       \item[$(ParAsc1, ParR)$]
%       \item[$(ParAsc1, EComL)$]
...
%       \item[$(ParAsc1, ClsL)$]
%       \item[$(ParAsc1, ClsR)$]
%       \item[$(ParAsc2, \_)$] $ParAsc2$ is unnecessary because it can be emulated by $ParAsc1$ and $ParCom$.
%       \item[$(ParCom, ParL)$]
%       \item[$(ParCom, ParR)$]
%       \item[$(ParCom, EComL)$]
%       \item[$(ParCom, EComR)$]
%       \item[$(ParCom, ClsL)$]
%       \item[$(ParCom, ClsR)$]
%       \item[$(ScpExtPar1, ParL)$]
%       \item[$(ScpExtPar1, ParR)$]
%       \item[$(ScpExtPar1, EComL)$]
%       \item[$(ScpExtPar1, EComR)$]
%       \item[$(ScpExtPar1, ClsL)$]
%       \item[$(ScpExtPar1, ClsR)$]
%       \item[$(ScpExtPar2, ParL)$]
%       \item[$(ScpExtPar2, ParR)$]
%       \item[$(ScpExtPar2, EComL)$]
%       \item[$(ScpExtPar2, EComR)$]
%       \item[$(ScpExtPar2, ClsL)$]
%       \item[$(ScpExtPar2, ClsR)$]
%       \item[$(CongPar, ParL)$]
%       \item[$(CongPar, ParR)$]
%       \item[$(CongPar, EComL)$]
%       \item[$(CongPar, EComR)$]
...
\begin{lemma}[Existence of a normal form]\label{existenceOfNormalForm}
...
% \begin{lemma}[Weak reduction to normal form]\label{mapToNormalForm}
%   Let $\equiv_{2}$ be the smallest congruence induced by the rules for scope extrusion and for $\alpha$ equivalence. $P \xrightarrow{\gamma}_{3} Q$ imply that there exist $N$ in normal form such that $P\equiv_{2} N$, $N \xrightarrow{\gamma}_{2} M$, $M\equiv Q$ and the derivation of $N \xrightarrow{\gamma}_{2} M$ does not use the rules for scope closure.
%   \begin{proof}
%     The proof is by induction on the derivation of $P \xrightarrow{\gamma} Q$. The last rule used can be:
%     \begin{description}
%       \item[$Out$]
% 	$\overline{x}y.P \xrightarrow{\overline{x}y} P$. For lemma \ref{existenceOfNormalForm}: there exists a process $N$ in normal form such that $P \equiv N$. For rule $Out$: $\overline{x}y.N \xrightarrow{\overline{x}y} N$. It also holds that $\overline{x}y.P$ is structurally congruent to $ \overline{x}y.N$ and that $\overline{x}y.N$ is in normal form.
%       \item[$Tau$] similar.
%       \item[$EInp$]
% 	$x(y).P \xrightarrow{xz} P\{z/y\}$. For lemma \ref{existenceOfNormalForm}: there exists a process $N$ in normal form such that $P \equiv N$ so:
% 	\begin{center}
% 	  \begin{tabular}{ll}
% 	    $\inferrule* {
% 	      P \equiv N
...
% 	    $\inferrule* [left=\bf{EInp}]{
% 	    }{
% 	      x(y).N \xrightarrow{xz} N\{z/y\}
% 	    }$
% 	  \end{tabular}
% 	\end{center}
% 	and $x(y).N$ is a normal form.
%       \item[$SInp$]:
% 	\begin{center}	
% 	  $\inferrule* [left=\bf{SInp}]{
% 	      P \xrightarrow{\gamma} P^{'}
% 	  }{
% 	    \underline{x(y)}.P \xrightarrow{x(y) \cdot \gamma} P^{'}
% 	  }$
% 	\end{center}
% 	for inductive hypothesis there exist processes $N,N^{'}$ in normal form such that $P \equiv_{2} N$, $N \xrightarrow{\gamma} N^{'}$ and $N^{'} \equiv P^{'}$. So:
% 	\begin{center}
% 	  \begin{tabular}{ll}
% 	      $\inferrule* {
% 		P \equiv_{2} N
% 	      }{
% 		\underline{x(y)}.P \equiv_{2} \underline{x(y)}.N
% 	      }$
% 	    &
% 	      $\inferrule* [left=\bf{SInp}]{
% 	      }{
% 		\underline{x(y)}.N \xrightarrow{x(y) \cdot \gamma} N^{'}
% 	      }$
% 	  \end{tabular}
% 	\end{center}
%       \item[$LComSeq$]:
...
% 	      P \xrightarrow{x(y) \cdot \gamma} P^{'}
% 	    \\
% 	      Q \xrightarrow{\overline{x}z} Q^{'}
% 	    \\
% 	      bn(\gamma) \cap fn(Q) = \emptyset
% 	  }{
% 	    P|Q \xrightarrow{\gamma\{z/y\}} P^{'}\{z/y\}|Q
% 	  }$
% 	\end{center}
% 	for inductive hypothesis there exist processes $N,N^{'},M,M^{'}$ such that $N,M$ are normal forms, $P \equiv_{2} N$, $N \xrightarrow{x(y) \cdot \gamma} N^{'}$, $N^{'} \equiv P^{'}$, $Q \equiv M$, $M \xrightarrow{\overline{x}z} M^{'}$ and $M^{'} \equiv Q^{'}$. So:
% 	\begin{center}	
% 	  \begin{tabular}{ll}
% 	      $\inferrule* {
% 		  P \equiv_{2} N
...
% 	    &
% 	      $\inferrule* [left=\bf{LComSeq}]{
% 		  N \xrightarrow{x(y) \cdot \gamma} N^{'}
% 		\\
% 		  M \xrightarrow{\overline{x}z} M^{'}
% 		\\
% 		  bn(\gamma) \cap fn(Q) = \emptyset
% 	      }{
% 		N|M \xrightarrow{\gamma\{z/y\}} N^{'}\{z/y\}|M^{'}
% 	      }$
% 	  \end{tabular}
% 	\end{center}
% 	If $N|M$ is not a normal form then $N$ or $M$ have unguarded restrictions at the top level, for scope extension rules there is a normal form structurally equivalent to $N|M$ and the conclusion holds because of rule $Cong$.
%       \item[$LCom$] similar.
%       \item[$Par$]:
% 	\begin{center}	
% 	  $\inferrule* [left=\bf{Par}]{
...
% 	  }$
% 	\end{center}
% 	for inductive hypothesis there exist processes $N,N^{'}$ in normal form such that $P \equiv N$, $N \xrightarrow{\gamma} N^{'}$, $N^{'} \equiv P^{'}$. So
% 	\begin{center}
% 	  \begin{tabular}{ll}
% 	    $\inferrule* {
% 	      P \equiv N
...
% 	  &  
% 	    $\inferrule* [left=\bf{Par}]{
% 		N \xrightarrow{\gamma} N^{'}
% 	      \\
% 		bn(\gamma) \cap fn(Q) = \emptyset
% 	    }{
% 	      N|Q \xrightarrow{\gamma} N^{'}|Q
% 	    }$
% 	  \end{tabular}
% 	\end{center}
% 	
...
% 	  }$
% 	\end{center}
% 	for inductive hypothesis there exist processes $N,N^{'}$ in normal form such that $Q \equiv N$, $N \xrightarrow{\gamma} N^{'}$, $N^{'} \equiv Q^{'}$. For transitivity $P \equiv N$.
%       \item[$Res$]:
% 	\begin{center}	
% 	  $\inferrule* [left=\bf{Res}]{
% 	      P \xrightarrow{\gamma} P^{'}
...
% 
% \begin{definition}
%   $\sim_{E}^{0}$ is the identity relation on the set of processes that have an empty semantic. $\sim_{E}^{i+1} \subseteq \mathbf{P}^{2}$ and for all $(P,Q)\in \sim_{E}^{i+1}$ it holds that:
%   \begin{itemize}
%     \item 
%       for all $P\xrightarrow{\gamma}P^{'}$ there exists a process $Q^{'}$ such that $Q\xrightarrow{\gamma}Q^{'}$ and $P^{'}\sim_{E}^{i}Q^{'}$
%     \item
%       for all $Q\xrightarrow{\gamma}Q^{'}$ there exists a process $P^{'}$ such that $P\xrightarrow{\gamma}P^{'}$ and $Q^{'}\sim_{E}^{i}P^{'}$
%   \end{itemize}
%   then we can define $\sim_{E}^{\omega}=\bigcup_{i\in \mathbb{N}} \sim_{E}^{i}$
% \end{definition}
% $\sim_{E}^{\omega}$ is a bisimulation, but it does not hold that $\sim_{E} \subseteq \sim_{E}^{\omega}$ because for example
% \begin{center}
%   \begin{tabular}{ll}
%       $A(x,y)\stackrel{def}{=} \overline{x}y.A$
%     &
%       $B(x,y)\stackrel{def}{=} \overline{x}y.B$
%   \end{tabular}
% \end{center}
% It holds that $A \sim_{E} B$ but it does not hold that $A \sim_{E}^{\omega} B$. So we can try a new definition
% \begin{definition}
%   Let $Sem_{0}$ be the set of processes that have an empty semantic and let $\mathbb{P}$ be the set of all processes. We define $\sim_{E}^{0}$ as the union of: $(Sem_{0})^{2}$, the identity relation on $\mathbb{P}$ and every pair of processes $(P,Q)$ such that 
%   \begin{itemize}
%     \item 
%       $P\xrightarrow{\gamma}P$ imply $Q\xrightarrow{\gamma}Q$
%     \item
%       $Q\xrightarrow{\gamma}Q$ imply $P\xrightarrow{\gamma}P$
%   \end{itemize} 
%   $\sim_{E}^{i+1} \subseteq \mathbb{P}^{2}$ and for all $(P,Q)\in \sim_{E}^{i+1}$ it holds that:
%   \begin{itemize}
%     \item 
%       for all $P\xrightarrow{\gamma}P^{'}$ there exists a process $Q^{'}$ such that $Q\xrightarrow{\gamma}Q^{'}$ and $P^{'}\sim_{E}^{i}Q^{'}$
%     \item
%       for all $Q\xrightarrow{\gamma}Q^{'}$ there exists a process $P^{'}$ such that $P\xrightarrow{\gamma}P^{'}$ and $Q^{'}\sim_{E}^{i}P^{'}$
%     \item 
%       if there exists a cycle $P \xrightarrow{\gamma_{1}} P_{1} \cdots P_{n} \xrightarrow{\gamma_{n}} P$ then there is a cycle $Q \xrightarrow{\gamma_{1}} Q_{1} \cdots Q_{n} \xrightarrow{\gamma_{n}} Q$ and the pairs $(P,Q),(P_{1},Q_{1}),\cdots,(P_{n},Q_{n})$ are in $\sim_{E}^{i}$.
%     \item 
%       the symmetric of the previous case.
%   \end{itemize}
%   then we can define $\sim_{E}^{\omega}=\bigcup_{i\in \mathbb{N}} \sim_{E}^{i}$
% \end{definition}
% 
