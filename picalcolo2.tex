...
\begin{definition}\index{$\alpha$ convertible}
  ...
  \begin{table}
    ...
    \\\\
	  $\inferrule*[left=AlpSub]{
	      P\equiv_{\alpha}Q
	    \\
	      x<y
	  }{
	    P\{x/y\}\equiv_{\alpha}Q\{x/y\}
	  }$
	&
      \\\\\hline
    \end{tabular}
...
We assume that there is a well order relation $\leq$ between the names. The condition $x<y$ in rule $AlpSub$ ensure that we can apply this rule only a finite amount of times thus any proof tree of $\alpha$ equivalence is finite. 

\begin{lemma}
  If $P\equiv_{\alpha}Q$ then there exists a proof of $P\equiv_{\alpha}Q$ such that $AlpSub$ is not the last rule used.
  \begin{proof}
    Suppose we have a proof of $P\equiv_{\alpha}Q$, if the last rule used is $AlpSub$ then 
  \end{proof}
\end{lemma}
