...
\begin{lemma}\label{multioutconstraintswithmarked}
  ...
  \begin{itemize}
    ...
    \item
      if $P\stackrel{\alpha}{\longmapsto}P$ then $\alpha$ cannot be an $\epsilon$
  \end{itemize}
%   In other terms:
%   \begin{center}
%     \synttree[
  ...
  \begin{center}
	...
	*\overline{x}c.P | Q\{a/d\} | R\{b/e\}  \stackrel{\overline{x}c}{\longmapsto} P | Q\{a/d\} | R\{b/e\}
	...
	x(f).S  \stackrel{xc}{\longmapsto} R\{c/f\}
      ...
      *\overline{x}c.P | Q\{a/d\} | R\{b/e\} | x(f).S  \stackrel{\epsilon}{\longmapsto} P | Q\{a/d\} | R\{b/e\} | S\{c/f\}
    }$
  \end{center}
...
\begin{proposition}
  ...
  \begin{proof}
    ...
    \begin{description}
      \item[base case]
    \end{description}
	If the depth is one then the rule used have to be one of: $EInp$, $Out$, $Tau$. These rules are also in table \ref{lowleveltransitionrelation} so we can derive $P \stackrel{\sigma}{\longmapsto}Q$.
    \begin{description}
      \item[inductive case]
    \end{description}
	  ...
	  \item[$\gamma_{i}=\overline{x}y$]:
	    ...
	    \begin{center}
		  ...
		  \cdots 
			      ...
			      \stackrel{\epsilon}{\longmapsto} P_{1}^{'}|Q_{1}^{'}$	  
	    \end{center}
	    we derive the transaction $ L_{i-1}|Q_{1} \stackrel{\tau}{\longmapsto} L_{i}|Q_{1}^{'}$ with rule $Com3L$, whether for the other transactions  we use the rule $Par1L$.
	  ...
	  \item[$\gamma_{i}=\overline{x}y$]:
	    ...
	    \begin{center}
		  ...
		  \cdots 
			      ...
			      \stackrel{\gamma_{k+1}}{\longmapsto} P_{1}^{'}|Q_{1}^{'}$	  
	    \end{center}
	    we derive the transition $ L_{i-1}|Q_{1} \stackrel{\tau}{\longmapsto} L_{i}|Q_{1}^{'}$ with rule $Com3L$, whether for the other transactions of the premises we use the rule $Par1L$.
...
\begin{proposition}
  Let $\rightarrow$ be the relation defined in table \ref{lowleveltransitionrelationearlyoutonly}. If there exist $L_{1}, \cdots, L_{k}$ and $\gamma_{1}, \cdots, \gamma_{k+1}$ with $k\geq 0$ such that 
  \begin{center}
    \begin{tabular}{lll}
      $P \stackrel{\gamma_{1}}{\longmapsto} L_{1}  \stackrel{\gamma_{2}}{\longmapsto} L_{2} \cdots L_{k-1} \stackrel{\gamma_{k}}{\longmapsto} L_{k} \stackrel{\gamma_{k+1}}{\longmapsto} Q$ 
    &
    ...
    \end{tabular}
  \end{center}
  then $P\xrightarrow{\sigma} Q$.
  \begin{proof}
    The proof is by induction on $k$ plus the depth of the derivation of the first low transition:
    \begin{description}
      \item[base case $k=0$]
    \end{description}
    ...
    \begin{description}
      \item[inductive case]
    \end{description}
	we have two cases: the first is $P\stackrel{\sigma}{\longmapsto} Q$ and the depth of the derivation is greater than one. In this case the last rule in the derivation can be: $Sum$, $Com1$, $Res$, $Par1L$, $Par1R$, $Cong$:
	\begin{description}
	  \item[$Sum$]:
		\begin{center}
		  $\inferrule* [left=\bf{Sum}]{
		      P_{1} \stackrel{\sigma}{\longmapsto} Q
		    }{
		      P_{1}+P_{2} \stackrel{\sigma}{\longmapsto} Q
		  }$ 
		\end{center}
	    for inductive hypothesis $P_{1} \xrightarrow{\gamma_{1}} Q$ and for rule $Sum$ $P_{1}+P_{2} \xrightarrow{\gamma_{1}} Q$.
	  \item[$others$] 
	    the other cases are similar to the previous since these rules are in common. 
	\end{description}
	The second case is that $k>0$. The last rule in the derivation of $P \stackrel{\gamma_{1}}{\longmapsto} L_{1}$ can be:
	\begin{description}
	  \item[$SOutLow$]:
	    \begin{center}
	      $\underline{\overline{x}y}.P_{1} \stackrel{\overline{x}y}{\longmapsto} *P_{1} \stackrel{\gamma_{2}}{\longmapsto} L_{2} \cdots L_{k-1} \stackrel{\gamma_{k}}{\longmapsto} L_{k} \stackrel{\gamma_{k+1}}{\longmapsto} Q$ 
	    \end{center}
	    Since $*P_{1}$ has a mark on top, the last rule used in a derivation of $*P_{1} \stackrel{\gamma_{2}}{\longmapsto} L_{2}$ has to be $Star$. This means that $P_{1} \stackrel{\gamma_{2}}{\longmapsto} L_{2}$ and so
	    \begin{center}
	      $P_{1} \stackrel{\gamma_{2}}{\longmapsto} L_{2} \cdots L_{k-1} \stackrel{\gamma_{k}}{\longmapsto} L_{k} \stackrel{\gamma_{k+1}}{\longmapsto} Q$ 
	    \end{center} 
	    which for inductive hypothesis implies $P_{1} \xrightarrow{\gamma_{2}\cdot \ldots \cdot \gamma_{k+1}} Q$. A proof of the conclusion is build by applying one of the rules $SOut$, $SOutTau$ or $SOutSeq$. We can apply one and only one of this rules for strong output thanks to lemma \ref{multioutconstraintswithmarked}.
		...
		\begin{center}
		  $P_{1} \stackrel{\gamma_{1}}{\longmapsto} L_{1} \stackrel{\gamma_{2}}{\longmapsto} L_{2} \cdots L_{k-1} \stackrel{\gamma_{k}}{\longmapsto} L_{k} \stackrel{\gamma_{k+1}}{\longmapsto} Q$ 
		\end{center}
		apply the inductive hypothesis to get $P_{1} \xrightarrow{\gamma_{1}\cdot \ldots \cdot \gamma_{k+1}} Q$. Now a proof of the conclusion can be
		\begin{center}
		  $\inferrule* [left=\bf{Sum}]{
