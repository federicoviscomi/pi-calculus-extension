...
A normal form always exists but it is not unique.

% \begin{lemma}
%   If $P$ is a normal form, $P \xrightarrow{\gamma} P^{'}$ and $\gamma$ is not a bound output then there exists a process $(\nu \tilde{x})Q$ such that $Q$ is a normal form, have no restrictions at the top level, $P \equiv (\nu \tilde{x})Q$, $(\nu \tilde{x})Q \xrightarrow{\gamma} Q^{'}$, $P^{'} \equiv Q^{'}$ and the last $|\tilde{x}|$  rules in the derivation of $(\nu \tilde{x})Q \xrightarrow{\gamma} Q^{'}$ are $Res$. $\tilde{x}$ can be empty.
%   \begin{proof}
%     If $P$ has no restrictions at the top level then the lemma holds. Otherwise 
%   \end{proof}
% \end{lemma}
% 
% \begin{lemma}
%   If $P$ is a normal form, $P \xrightarrow{\overline{a}(b)} P^{'}$ then there exists a process $(\nu \tilde{x})Q$ such that $Q$ is a normal form, have no restrictions at the top level, $(\nu \tilde{x})Q \xrightarrow{\overline{a}(b)} Q^{'}$, $P^{'} \equiv Q^{'}$ and the last rule used in the derivation of $(\nu \tilde{x})Q \xrightarrow{\overline{a}(b)} Q^{'}$ is the rule that opens the scope of $b$.
%   \begin{proof}
%     
%   \end{proof}
% \end{lemma}
...
\begin{lemma}[Reduction to normal form]\label{mapToNormalForm}
  $P \xrightarrow{\gamma} Q$ imply that there exist $N$ in normal form such that $P\equiv N$, $N \xrightarrow{\gamma} M$, $M\equiv Q$ and the depth of the inference tree of $N \xrightarrow{\gamma} M$ is not greater than the depth of the inference tree of $P \xrightarrow{\gamma} Q$.
  \begin{proof}
	...
	\end{center}
	for inductive hypothesis there exist processes $N,N^{'}$ in normal form such that $P \equiv N$, $N \xrightarrow{\gamma} N^{'}$ and $N^{'} \equiv P^{'}$. So:
	\begin{center}
	  \begin{tabular}{ll}
	      $\inferrule* {
		P \equiv N
	      }{
		\underline{x(y)}.P \equiv \underline{x(y)}.N
	      }$
	    &
	      $\inferrule* [left=\bf{SInp}]{
	      }{
	...
	\end{center}
	for inductive hypothesis there exist processes $N,N^{'},M,M^{'}$ such that $N,M$ are normal forms, $P \equiv N$, $N \xrightarrow{x(y) \cdot \gamma} N^{'}$, $N^{'} \equiv P^{'}$, $Q \equiv M$, $M \xrightarrow{\overline{x}z} M^{'}$ and $M^{'} \equiv Q^{'}$. So:
	\begin{center}	
	  \begin{tabular}{ll}
	      $\inferrule* {
		  P \equiv N
	  ...
	  \end{tabular}
	\end{center}
NON FUNZIONA PERCHE' $N|M$ POTREBBE NON ESSERE UNA FORMA NORMALE!
      \item[$LCom$] similar.
      \item[$Par$]:
	\begin{center}	
	  $\inferrule* [left=\bf{Par}]{
...
% 
% $P \xrightarrow{\gamma}_{3} P^{'}$ implica $P\equiv Q \xrightarrow{\gamma}_{2} P^{'}$ implica $P\equiv Q\equiv_{\alpha} N \xrightarrow{\gamma}_{2} P^{''}\equiv P^{'}$, $N$ e' in forma normale e $N\xrightarrow{\gamma}_{2} P^{''}$ non usa $Cls, ClsSeq$.
