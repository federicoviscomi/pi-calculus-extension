...
%	  	  y\notin fn((\nu z) P)
	...
	\begin{description}
	  \item[base case] $R$ can be only $SInpLow$, so 
	    \begin{center}
	  ...
	  \item[inductive case] $R$ can be:
		...
		\end{center}
		given that $L_{1}$ has a restriction at the top level, all the other intermediate processes $L_{2}, \cdots, L_{k}$ have the same restriction at the top level. So the last rule used to prove $L_{i}\stackrel{\gamma_{i+1}}{\longmapsto} L_{i+1}$ for each $1\leq i \leq k-1$ and we can build the following chain of transitions:
		\begin{center}
		  $P_{1} \stackrel{\gamma_{1}}{\longmapsto} L_{1}^{'} \stackrel{\gamma_{2}}{\longmapsto} L_{2}^{'} \cdots L_{k-1}^{'} \stackrel{\gamma_{k}}{\longmapsto} L_{k}^{'}$ 
		\end{center}
		then apply the rule $Low$ and the inductive hypothesis to get $P_{1} \xrightarrow{\gamma_{1}\cdot \ldots \cdot \gamma_{k}} L_{k}^{'}$. The last rule used to derive $L_{k} \stackrel{\gamma_{k+1}}{\longmapsto} Q$ can be one of: $Cong3$, $Res$ or $Open$. Suppose it is $Cong3$(the other cases are similar)NO NO NO
		\begin{center}
		  $\inferrule* [left=\bf{Cong3}]{
		      (\nu z)L_{k}^{'} \stackrel{\gamma_{k+1}}{\longmapsto} T
		    \\
		      T \equiv Q
		  }{
		...
		\end{center}
		a proof of the conclusion can be
		\begin{center}
		  $\inferrule* [left=\bf{Res}]{
			P_{1} \xrightarrow{\gamma_{1}\cdot \ldots \cdot \gamma_{k}} L_{k}^{'}
		      \\
			z\notin n(\gamma_{1}\cdot \ldots \cdot \gamma_{k})
		    }{
		      (\nu z)P_{1} \xrightarrow{\gamma_{1}\cdot \ldots \cdot \gamma_{k}} (\nu z)L_{k}^{'}
		  }$
		\end{center}
	      \item[$Opn$]
	      \item[$Par1$]
