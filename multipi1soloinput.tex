...
In the following section the symbol $\rightarrow$ will refer to the late semantic with structural congruence of multi $\pi$ calculus with strong input which is illustrated in table \ref{multipisoloinputlatewith}. Also we consider a structural congruence without the rules $P|0\equiv 0$ and $P+0\equiv 0$
...
\begin{proposition}
  If $P \xrightarrow{\gamma} Q$ then there exists a process $N$ in normal form such that $N \xrightarrow{\gamma} Q$ and $P\equiv N$
\end{proposition}

\begin{lemma}[Inversion lemma for structural congruence]
  :\begin{description}
    \item[$Output$]
...
\begin{proposition}
  ...
  \begin{proof}
	...
	\begin{description}
	  ...
	  \item[$Cong$]
	    A process structurally congruent to $\overline{x}y.P$ must be in the form $\overline{x}y.R$ where $P \equiv R$ so $\overline{x}y.P \xrightarrow{\overline{x}y} R$.
	\end{description}
      \item[Input prefix]:
      \item[Tau prefix]
	Let $P \dot{\sim} Q$ and let $\tau.P \xrightarrow{\alpha} P^{'}$. The last rule used in the derivation of this transition can be:
	\begin{description}
	  \item[$Tau$]
	    $\tau.P \xrightarrow{\tau} P$ and $\tau.Q \xrightarrow{\tau} Q$ and $P \dot{\sim} Q$
	  \item[$Cong$]
	    A process structurally congruent to $\tau.P$ must be in the form $\tau.R$ where $P \equiv R$ so $\overline{x}y.P \xrightarrow{\overline{x}y} R$.
	\end{description}
    \end{description}
  \end{proof}
\end{proposition}
