...
\begin{definition}\index{syntactic substitution}
  $P\{b/a\}$ is the syntactic substitution of name $b$ for a different name $a$ inside a $\pi$ calculus process, and it consists in replacing every free occurrences of $a$ with $b$. If $b$ is a bound name in $P$ then we do not allow substitution. The substitution is defined in table \ref{syntacticsubstitution}.
\end{definition}
  ...
  \begin{table}
	...
	if $b\notin bn(def(A))$
      \\
	$(A(x_{1}, \cdots, x_{n}))\{b/a\}=A(x_{1}\{b/a\}, \cdots, x_{n}\{b/a\})$
      \\\\
	$(Q+R)\{b/a\}\; =\; Q\{b/a\} + R\{b/a\}$
      \\\\
	$(Q|R)\{b/a\}\; =\; Q\{b/a\} | R\{b/a\}$
...
\begin{definition}
  A \emph{strong late simulation} is a binary relation $\mathbf{S}$ on processes such that for each process $P$ and $Q$, $P\mathbf{S}Q$ implies:
  \begin{itemize}
    \item 
      if $P \xrightarrow{\alpha}_{L} P^{'}$ and $\alpha$ is a free action then there exists a process $Q^{'}$ such that $Q \xrightarrow{\alpha}_{L} Q^{'}$ and $P^{'}\mathbf{S}Q^{'}$
    \item
      if $P \xrightarrow{x(y)}_{L} P^{'}$ and $y\notin n(P)\cup n(Q)$ then there exists a process $Q^{'}$ such that $Q \xrightarrow{x(y)}_{L} Q^{'}$ and $Q\mathbf{S}Q^{'}$ and for all $w$ $P^{'}\{w/y\}\mathbf{S}Q^{'}\{w/y\}$
    \item
...
\begin{definition}
  A \emph{strong late bisimulation} is a binary relation $\mathbf{S}$ on processes such that both $\mathbf{S}$ and its inverse are simulations. We define the relation $\dot{\sim}$ of \emph{strong late bisimilarity} such that $P\dot{\sim}Q$ means that there exists a strong late bisimulation $\mathbf{S}$ such that $P\mathbf{S}Q$
\end{definition}
...
\begin{example}
  If $z\notin fn(R)\cup \{x\}$ and we are in a late semantic context then we want $x(y).R$ and $(z)x(y).R$ to be strongly bisimilar according to \cite{milnerparrowwalker}:
\end{example}
